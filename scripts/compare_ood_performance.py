...
def cli(
    ...
    out_dir = dm.fs.join(base_save_dir, "dataframes", f"compare_performance_{representation_ood.lower()}_distance", sub_save_dir)
    ...
    X_ood, mask_ood = featurize(
        smiles,
        representation_ood,
        standardize_fn=DEFAULT_PREPROCESSING[representation],
        disable_logs=True,
    )
    mask = [idx for idx in mask if idx in mask_ood]
    y = y[mask]
    ...
    for seed in range(n_seeds):
        ...
        X_train, X_test, X_ood_train, X_ood_test, y_train, y_test = train_test_split(X, X_ood, y, test_size=0.2, random_state=seed)
        X_train, X_val, X_ood_train, _, y_train, y_val = train_test_split(X_train, X_ood_train, y_train, test_size=0.2, random_state=seed)
        ...
        dist_train_, dist_test_ = compute_knn_distance(X_ood_train, [X_ood_train, X_ood_test])
    ...
    dist_scr = load_distances_for_downstream_application(
        "virtual_screening", representation_ood, dataset, update_cache=True
    )
    dist_opt = load_distances_for_downstream_application(
        "optimization", representation_ood, dataset, update_cache=True
    )
